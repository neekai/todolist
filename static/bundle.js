...
var _TodoListEntry = __webpack_require__(27);

var _TodoListEntry2 = _interopRequireDefault(_TodoListEntry);
...
var TodoList = function (_React$Component) {
    ...
    _createClass(TodoList, [{
        ...
        key: 'fetchTodosFromDB',
        value: function fetchTodosFromDB() {
            var _this2 = this;

            _axios2.default.get('/api/todos').then(function (data) {
                console.log('we got the data from fetchTodos..', data);
                console.log('here', [].concat(_toConsumableArray(data.data)));
                var todosArr = [];
                data.data.forEach(function (todo) {
                    todosArr.push(todo.todo);
                });
                _this2.setState({
                    todos: todosArr
                }, function () {
                    console.log('this.state.todos = ', _this2.state.todos);
                });
            });
        }
    }, {
        ...
        value: function handleSubmit(e) {
            e.preventDefault();
            this.setState({
                todos: [].concat(_toConsumableArray(this.state.todos), [this.state.todo])
            });
            e.target.reset();
            _axios2.default.post('/api/todos', {
        ...
        value: function render() {
            ...
            return _react2.default.createElement(
                ...
                this.state.todos.map(function (todo, index) {
                    return _react2.default.createElement(_TodoListEntry2.default, { todo: todo, index: index, key: index, deleteTodo: _this3.deleteTodo });
                })
            );
        }
    }]);
...
var TodoListEntry = function TodoListEntry(_ref) {
    var todo = _ref.todo,
